...
% To use this code set all the parameters that you need, these will most likely
...
% Once entered, wait, it may take awhile depending on the grid size and simLength
% After finishing some figures and visualizations will generate with relevant
% information. 
...
% Wind Parameters
prob_wind_spawn = 0.00; % Percent chance that a cell on the wind grid develops wind
        ...
        for col = 2:col_count + 1
            ...
            if(current_pollen_conc > 0 || plant_count > 0)
                ...
                % Adding and subtracting the pollen for this current location
                    ...
                    % pollen moving in greater amounts towards neighbor in wind
                    % direction
            ...
            % Moves in a similar way to pollen. More animals towards neighbor
            ...
            % Empty Cell
            % becomes plant if neighboring a pollinated plant, otherwise will
            % become animal if animals are present, otherwise will remain empty
            ...
            % Animal Cell
            % Will reproduce if conditions are met, adding population to the
            % animal aggregate, otherwise will become animal or empty if not
            ...
            % Plant Cell
            % Chance to die and eliminate all plants at this cell, if neighboring
            % pollinated plant than chance to grow another plant in the aggregate
            % If neighboring animal than has a chance to become pollinated plant
            ...
            % Creates new plants and then becomes a refractory plant, or empty
            ...
            % Will produce no pollen and cannot reproduce, will become a normal
            ...
            % Wind functions as a very simple CA simulation done in another grid
            % Every timestep empty cells in this grid have a chance to become
            % some sort of wind, empty cells neighboring a wind cell will then
            % become the mode of all their wind neighbors.
...
map = [empty_color; plant_color; pollplant_color; refracplant_color; animal_color];
...
disp("Drawing...");
% This display loop can be altered however you see if in order to help debug
% and better see how certain things are working over time, however, due to it's
% slow run time and conflict between different visualizations it is currently
% all commented out.
...
title_text = text(states_axes,0.1,1.1, "Cellular Automata Grid","Units","Normalized");
time_counter_text = text(states_axes,0.1,1.025,"Current Time Step: " + ...
string(i), 'Units', 'Normalized');
animal_counter_text = text(states_axes,0.9,0,"Animals: " + ...
